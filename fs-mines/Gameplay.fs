...
type GameState = 
    ...
    member x.Covers = x.Moves |> Seq.head
    
    member x.Evaluate(mineField : MineField) = 
        { HasFailed = mineField.Mines |> Seq.exists (fun mine -> not (x.Covers.IsCovered(mine)))
...
module Game = 
    let Start(mineField : MineField) = GameState.Create(Covers.Create(mineField.Size))
    let Undo(current : GameState) = current.Undo
