...
type MineField = {Size: Size; Mines: Set<Point>; Warnings: Map<Point, int>} with
    member x.HasMineAt( point:Point) = 
        x.Mines.Contains(point)
    ...
    member x.HasWarningAt(point:Point) = 
       x.Warnings.ContainsKey(point)

    member x.IsEmptyAt(point:Point) =
        not(x.HasMineAt(point)) && not (x.HasWarningAt(point))
 

module MineFiedlBuilder =
    let private CreateRandomMines(minesCount:int, size:Size) =
        let rnd = System.Random()
        Seq.initInfinite(fun _ -> {X=rnd.Next(size.Width); Y=rnd.Next(size.Height)}) 
        |> Seq.take(minesCount)
        |> Set
    ...
    let public GenerateRandom(minesCount: int, size: Size) =
        ...
        {Size = size; Mines = mines; Warnings = warnings}

type Covers with
    member x.UncoverDeep( mineField: MineField, point: Point ) =
        let rec UncoverDeep ( covers: Covers, mineField: MineField, point: Point ) = 
            let seed = x.Uncover(point)
            point.Neighbours(mineField.Size) |>  Seq.fold (fun current neighbor -> UncoverDeep(current, mineField, neighbor )) seed
        if (not(x.IsCovered(point))) then x else
        if (not(mineField.IsEmptyAt(point))) then x.Uncover(point) else 
        UncoverDeep(x, mineField, point)