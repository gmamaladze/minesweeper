...
type Direction = {DX:int; DY:int} with 
    static member (+) (first:Direction, second:Direction) = {DX=first.DX + second.DX; DY=first.DY + second.DY}
    static member (+) (first:Point, second:Direction) = {X=first.X + second.DX; Y=first.Y + second.DY}
    static member Up : Direction = {DX=0; DY=(-1)}
    static member Down = {DX=0; DY=1}
    static member Left = {DX=(-1); DY=0}
    static member Right = {DX=1; DY=0}
    static member All = [
            Direction.Up; 
